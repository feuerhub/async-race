  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(startEngine.fulfilled, (state, action) => {
      ...
      const index = state.entities.findIndex(
        (engine) => engine.id === action.payload.id,
      );
      if (index !== -1) {
    ...
    builder.addCase(stopEngine.fulfilled, (state, action) => {
      ...
      state.entities = state.entities.filter(
        (engine) => engine.id !== action.payload,
      );
    });
...
export const selectAllEngineStatuses = (state: RootState) =>
  state.engine.entities;
export const selectEngineById = (id: number) => {
  (state: RootState) =>
